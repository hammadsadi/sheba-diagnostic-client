    ...
    <div className="flex flex-col max-w-sm mx-4 my-6 shadow-lg">
      ...
      <div className="flex flex-col items-center justify-center p-8 rounded-b-lg bg-primary text-white">
        <img
          src={health?.image}
          alt=""
          className="w-16 h-16 mb-2 -mt-16 bg-center bg-cover object-cover rounded-full  border-2"
        />
        <p className="text-xl font-semibold leading-tight">
          {health?.doctorName}
        </p>
        <p className="text-sm uppercase">{health?.position}</p>
      </div>
    </div>
  );
};

HealthTipsItem.propTypes = {
  health: PropTypes.object,
};

export default HealthTipsItem;
